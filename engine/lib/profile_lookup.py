...
profile. The waterfall starts with the person's own Reddit profile — that is where disclosure
is most likely — then escalates to web search if the profile does not reveal anything.

  Tier 1  Reddit profile   (/user/{name}/about.json) — free, fast, but 403 since mid-2025
  Tier 2  Exa search       ("{username}" founder OR CEO) — paid (~$0.01), finds linked presence
    ...
    try:
        ...
        if all_links:
            return {
                ...
                "bio": " | ".join(bio_snippets[:2]) if bio_snippets else None,
                "source": "exa",
                "signal": "social/professional profile linked to username",
            }
