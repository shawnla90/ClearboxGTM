...
```bash
python3 engine/replies.py scaffold --ops data/ops_classified.json --out data/suggested_replies.json
# the agent writes every empty reply slot, then:
python3 engine/replies.py check data/suggested_replies.json --ops data/ops_classified.json
python3 engine/replies.py sheet --ops data/ops_classified.json --replies data/suggested_replies.json --sheet-id <id>
python3 engine/replies.py angles --ops data/ops_classified.json --replies data/suggested_replies.json --out data/engage_angles.json
```
...
- `../../engine/replies.py` — the module this wraps: scaffold, check, sheet, angles
- `../reddit-engage/` — the interactive approval loop; expand a GO template into a full comment there
- `../personalization/` — the 3-variable model, applied at template length
- `../slack-digest/` — renders the angles file this skill exports
- `../../playbooks/how-to-win-on-reddit.md` — why value-first and no-links are non-negotiable
- `../../playbooks/automation-boundaries.md` — every send is a human pressing send
